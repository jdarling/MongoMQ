...
var MongoMQ = exports.MongoMQ = function(options, callback){
  ...
  var setupProperties = function(){
    ...
    self.__defineGetter__('listening', function(){
      return mongoConnection.active||mongoConnection._open;
    });
    ...
    if(typeof(partialCallback)==='function'){
      (function(partialCallback, completeCallback){
        ...
        var completeTest = function(call_err, next){
              ...
              self.mongoConnection.collection(self.options.collectionName, function(err, collection){
                collection.find(completeFilter).count(function(err, count){
                  if(count!==numExpected){
                    process.nextTick(function(){
                      completeTest(call_err, next);
                    });
                  }else{
                    partialListener.stop();
                    completeListener.stop();
                    completeCallback(call_err, results, next);
                  }
                });
              });
            };
        ...
        if(typeof(completeCallback)!=='function'){
          ...
          doComplete = function(err, data, next){
            ...
            completeTest(errors.length>0?errors:null, next);
          };
        }
        var tmpOptions = validateListenerArguments(messageId, {partialOnly: true, hereOnOut: false}, partialCallback);
        ...
        tmpOptions = validateListenerArguments(messageId, {completeOnly: true, hereOnOut: false}, completeCallback);
...
MongoMQ.prototype.removeListener = function(event, handler){
  ...
  if((idx!==false)&&(idx>-1)){
    self.listeners[idx].stop(listenerStopped);
    return true;
  }
  return false;
};
