...
QueueMonitor.prototype.start_usingNextObject = function(startupCallback){
  ...
  mq.collection(mq.options.queueCollection, function(err, collection){
    collection.insert({fix: cursorFixId}, {w: 1}, function(){
      mq.tailedCursor(mq.options.queueCollection, {$or: [{handled: false, event: event}, {handled: {$exists: false}, event: event}, {fix: cursorFixId}]}, {$natural: 1}, function(err, cursor){
        if(err){
...
QueueMonitor.prototype.start_usingStreams = function(startupCallback){
  ...
  mq.collection(mq.options.queueCollection, function(err, collection){
    if(collection){
      collection.insert({fix: cursorFixId}, {w: 1}, function(){
        mq.tailedCursorStream(mq.options.queueCollection, {$or: [{handled: false, event: event}, {handled: {$exists: false}, event: event}, {fix: cursorFixId}]}, {$natural: 1}, function(err, stream){
          if(!stream){
