          ...
          if(self.options.passive||mq.options.passive){
            ...
            collection.findAndModify(record, {$natural: -1}, set, {w: ((mq.db||{}).serverConfig||{}).replicaSet?self.options.writeConcern||mq.options.writeConcern||mq.db.serverConfig.servers.length:1}, function(err, data){
              if(data||err){
                ...
                // Different versions of the mongodb driver use different return
                // object formats for findAndModify. Found data is returned directly
                // in early versions and as .value in later versions.
                mq.emitter.emit('recieved', data.value||data);
                callback(err, (data.value||data||{}).data, next);
              }else{
